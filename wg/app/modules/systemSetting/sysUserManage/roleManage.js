    ...
    var myTable,
        ...
        vm = avalon.define({
            ...
            showAddWin: function (isEdit,data) {
                var url = window.currentApiUrl.systemSet.sysUser.roleEdit;
                if (!isEdit) {
                    url = window.currentApiUrl.systemSet.sysUser.roleAdd;
                    vm.role = defaultData();
                    setTimeout(function () {
            ...
            showDisenableWin: function (id) {
                cMessage.showAlert({
                    ...
                    onOk: function () {
                        jqueryAjax.post(
                            window.currentApiUrl.systemSet.sysUser.disEnableRole,
                            {roleId: id},
                            function (response, msgType, isOk) {
                                myTable.draw();
                                cMessage.showPopup({
            ...
            showEnableWin: function (id) {
                cMessage.showAlert({
                    ...
                    onOk: function () {
                        jqueryAjax.post(
                            window.currentApiUrl.systemSet.sysUser.enableRole,
                            {roleId: id},
                            function (response, msgType, isOk) {
                                myTable.draw();
                                cMessage.showPopup({
            ...
            showDeleteWin: function (id) {
                cMessage.showAlert({
                    ...
                    onOk: function () {
                        jqueryAjax.post(
                            window.currentApiUrl.systemSet.sysUser.roleDel,
                            {roleId: id},
                            function (response, msgType, isOk) {
                                myTable.draw();
                                cMessage.showPopup({
        ...
        baseTableOpts = {
            ...
            ajax: {
                url: window.currentApiUrl.systemSet.sysUser.roleList,
                type: 'GET',
                data: function (data) {
            ...
            operationCallback: function (e, rowData) {
                if (rowData) {
                    ...
                    if (target.hasClass('icon edit')) {
                        ...
                        avalon.router.go('app.systemSetting.roleManage.roleMenu',{roleId:rowData.id});
                    }else if (target.hasClass('icon users blue')) {
                        avalon.router.go('app.systemSetting.roleManage.roleUsers',{roleId:rowData.id});
